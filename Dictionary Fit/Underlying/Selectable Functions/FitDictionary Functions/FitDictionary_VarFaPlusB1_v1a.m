...
classdef FitDictionary_VarFaPlusB1_v1a < handle
...
% Constructor
%==================================================================  
function obj = FitDictionary_VarFaPlusB1_v1a()              
end
...
function [IMG,err] = FitDictionary(obj,IMG0)     
    ...
    %----
    % load('I:\25031813 (T1B1LungFb_Rws)\NEW_DATA_SMOOTHED_CROPPED.mat')
    % ImArray = squeeze(data.sig(:,:,:,1:3));
    % obj.B1Map.Im = flip(permute(obj.B1Map.Im,[3 2 1 4 5 6]),1);
    % obj.B1Map.Im = obj.B1Map.Im(data.ind2,data.ind1,data.ind3);
    % ImSize = size(ImArray);
    %----
