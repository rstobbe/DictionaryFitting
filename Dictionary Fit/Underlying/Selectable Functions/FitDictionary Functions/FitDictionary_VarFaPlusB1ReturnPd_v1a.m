...
classdef FitDictionary_VarFaPlusB1ReturnPd_v1a < handle
...
% Constructor
%==================================================================  
function obj = FitDictionary_VarFaPlusB1ReturnPd_v1a()              
end
...
function [IMG,err] = FitDictionary(obj,IMG0)     
    ...
    DictArrayMean = obj.DictObj.DictArrayMean(ind);
    DictArrayMean = reshape(DictArrayMean,ImSize(1:3));    

    Comp1 = ImArrayMean./DictArrayMean;
    Comp2 = Comp1 ./ RelB1Map;
    ...
    Im = cat(4,T1Map,RelB1Map,Comp1,Comp2,Dist);
    
    DispType = 'map';
    DispWid = [min(T1Map(:)) max(T1Map(:))];
    PixDim = IMG0{1}.IMDISP.ImInfo.pixdim;
    Vox = IMG0{1}.IMDISP.ImInfo.vox;
    IMG =  AddCompassGenericInfo(Im,'T1MapPlus',obj,PanelOutput,DispType,DispWid,PixDim,Vox);     
end
